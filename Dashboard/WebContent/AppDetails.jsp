	...
	<html>
		...
		<table id = "mainTable" align = "center">
			...
			<%for (int j = 0; j < ctr; j++) 
			{ 	
				if((col[j][6].equalsIgnoreCase("Applicant")) && (col[j][3].equalsIgnoreCase("Pending"))) // if the groupName is "Applicant"
				{
					if(col[j][9] != null)
					{
			...
			%>
				<tr style = "color: #ff8080; background-color: #1E90FF;">
			...
			<%
				...
				else if(col[j][6].equalsIgnoreCase("Applicant") && !(col[j][1].toLowerCase().contains("zoning"))) // if the groupName is "Applicant"
				{
					if(col[j][9] != null)
					{
						sumOfCompletedDays = Integer.parseInt(col[j][9]) + sumOfCompletedDays;
			...
			<%
				...
				else if((col[j][3].equalsIgnoreCase("Pending")) && !(col[j][1].toLowerCase().contains("zoning"))) // if the TaskStatus is "Pending"
				{
			%>
			...
			<%
				...
				else if (!(col[j][1].toLowerCase().contains("zoning"))) // everything else 
				{
			%>
